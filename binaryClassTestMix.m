...
function [perfMat,numberType] = binaryClassTestMix(class1,class2,blockTagsAC,features,folderpath,nameTag,featureNum)
...
numberType0 = zeros(length(participant),4);
...
%     if r < 10
%     testGroupNext = (crossInd == r+1);
%     else
%         testGroupNext = (crossInd == 1);
%     end
%     selectGroup = ~testGroupNext;
    testGroup = (crossInd == r);%&(~testGroupNext)
    trainGroup = ~testGroup;
    
    
    trainISPC = samples(:,trainGroup);
    nca = fscnca(trainISPC' ,(sampleTags(selectGroup)));% Feature selection out of features from all types
    selidxvalue = maxk(nca.FeatureWeights,featureNum);
    selidx = find(ismember(nca.FeatureWeights,selidxvalue));
    % Write the number of each type's selected features
    numberType0(p,1) = numberType0(p,1) + sum(selidx<=featureNum,'all');
    numberType0(p,2) = numberType0(p,2) + sum(selidx<=featureNum*2&selidx>featureNum,'all');
    numberType0(p,3) = numberType0(p,3) + sum(selidx<=featureNum*3&selidx>featureNum*2,'all');
    numberType0(p,4) = numberType0(p,4) + sum(selidx>featureNum*3,'all');
    % Train and test the classification model
    trainISPC0 = trainISPC';
    SVMmodel = fitcsvm(trainISPC0(:,selidx),(sampleTags(trainGroup)));
    [prediction,prescores] = predict(SVMmodel,(samples(selidx,testGroup))');
